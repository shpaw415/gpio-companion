...
- Dashboard Devices T3 tab (`/devices/t3`) embeds `https://t3-<slug>.gpio-companion.com` in a keep-alive iframe (hidden on other routes so the in-app T3 page is not lost); device selector picks the paired Pi; primary nav stays Project / Devices / Profile
