...
- T3 Code pairing and GitHub token are managed from the dashboard; OpenCode uses gpio-companion credits via a dashboard AI proxy (Workers AI binding). first-setup bakes `GPIO_AI_KEY` and the OpenCode provider URL. Empty credits return 402. After claim the dashboard runs `t3 start`, shows the web pairing URL, then `t3 service install`
