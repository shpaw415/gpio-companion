...
- Host run (locked 2026-09-12; sketch list 2026-09-12): async `POST /v1/run` `{ dir }` compiles Arduino-style C on the Pi and runs `setup`/`loop` on companion GPIO (physical pins, libgpiod). `GET /v1/run` is `{ running, log, last }`; `GET /v1/run/sketches` lists `~/projects/<repo>/host/<name>/`; `POST /v1/run/stop` kills the process. Same auth as flash. Project UI **Run on board** picks a sketch name. Skill `gpio-host`. Not a drop-in USB Arduino sketch.
