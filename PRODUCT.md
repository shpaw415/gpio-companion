...
- On-device updater `scripts/update-script.sh` pulls the repo, refreshes `opencode/skills` and `opencode/preferences`, fetches the dashboard device public key, upgrades T3 Code when npm `t3@latest` is ahead of the installed package (then `t3 service install` in the GPIO user's systemd session — linger + `user@UID`, from that user's home so T3 does not mkdir the installer cwd under `/root`; `npm install -g t3@latest --allow-scripts=msgpackr-extract,node-pty` — and OpenCode-only T3 providers), runs `opencode upgrade`, and rebuilds/restarts the gpio-companion server when those trees or the registered public key change
