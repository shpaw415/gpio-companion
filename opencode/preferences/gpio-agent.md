...
- **Arduino proxy:** `GET http://127.0.0.1:4150/v1/arduino-proxy`. If connected, write `~/projects/<repo>/host/arduino-proxy-<name>/` (Arduino pin numbers, skill `gpio-arduino-proxy`) and `POST /v1/run`. Do not flash over a live proxy unless asked. User flashes slave firmware from Devices → Flash Arduino as proxy (`POST /v1/flash/proxy`). AVR is 5V; SAMD/ESP32 are 3.3V.
...
- Project git (electronics clones in `~/projects/<name>`): never commit feature work on `main`. New feature → `git checkout -b feat/<kebab>`, write `pcb/` (`circuit.json` and `preview.svg` when possible), `breadboard/diagram.json` (Wokwi plug map; skill `gpio-breadboard`), `technical/` (sheets), `host/<name>/` (gpio-host C), `firmware/<name>/` (USB Arduino C), then `git add`, commit, `git push -u origin feat/<kebab>`. When the slice is done, ask **Want to save these changes to main?** Yes (save / keep / merge / yes): checkout `main`, merge the feature branch, `git push origin main`, stay on `main`. No: leave the branch. Never force-push `main`. No GitHub PRs. Dashboard **Save to GitHub** pushes the current checkout and is not the merge gate; you merge when the user asks you to save. Dashboard viewers read GitHub `main`. New-repo `.gpio-companion` watermark may land on `main` as bootstrap.
