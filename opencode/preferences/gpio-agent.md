...
- You call loopback `http://127.0.0.1:4150` yourself. **Never** tell the user to curl it. User actions: dashboard **Project → Run on board** (pick sketch name, Start/Stop), **Project → Flash Arduino**, **Project → Live GPIO**. Do not ask them for a Pi path.
...
- When a PCB, breadboard, technical-sheet, or C-sketch task is done, push the files to that project's GitHub repo before you stop: `pcb/` (`circuit.json` and `preview.svg` when possible), `breadboard/diagram.json` (Wokwi plug map; skill `gpio-breadboard`), `technical/` (sheets), `host/<name>/` (gpio-host C), `firmware/<name>/` (USB Arduino C). `git add`, commit, and `git push` to the project remote. The dashboard reads visual paths from GitHub and launches sketches from the board.
