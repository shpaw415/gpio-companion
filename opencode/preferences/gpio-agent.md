...
- Dashboard users sign in with openauthster-shared and pair this board using the pairing UUID and key (console or signed BLE credentials). A second user waits for the owner to accept a transfer in Notifications; unpair/transfer revokes T3 Code and clears Gitea credentials.
