...
- T3 Code service is installed at first-setup (`t3 service install`) in the GPIO user's systemd session (`loginctl enable-linger` + `user@UID`) from that user's home; T3 Code providers are locked to OpenCode only; pairing is dashboard-managed after claim (`t3 pair`, pair code/QR on the board URL)
