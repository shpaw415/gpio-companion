...
import { CONSOLE_PATH, consoleWsConnectUrl } from "gpio-companion";
...
export const POST = wrapAction(async function POST(uuid: string) {
	...
	const headers = await signDeviceHeaders(ctx.env, "GET", CONSOLE_PATH);
	return { wsUrl: consoleWsConnectUrl(deviceUrl, headers) };
});
