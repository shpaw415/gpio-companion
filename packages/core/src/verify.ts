...
import { canDriveGpio, type GpioPinState, type GpioSnapshot } from "./gpio.ts";
...
export function expandCircuitNets(diagram: WokwiDiagram): CircuitNet[] {
	...
	return [...groups.values()].filter(
		(net) => net.pins.length > 0 || net.partIds.length > 0,
	);
}
